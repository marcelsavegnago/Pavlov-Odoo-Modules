...
class ProjectScrumRelease(models.Model):
     ...
     name = fields.Char(string="Release Name",
                        required=True)
     description = fields.Text(string="Description",
                               help="Provides and area for text describing the release.")
     release_date = fields.Date(string="Release Date",
                                help="The official date the release was put into production.")
     project_tasks = fields.Many2many('project.task',
                                    relation='fixedversion_task_rel',
                                    column1='version_id',
                                    column2='task_id',
                                    string="Project Tasks",
                                    help="List the completed Project Tasks that were included in the Release.")
     ...
     release_notes = fields.Text(string="Release Notes",
                                 help="Auto populates when the release is completed using the Tasks. If text exists, the auto populated notes get appended to this field.")
     ...
     def release_version_and_new(self):
         ...
         self.color = 10
         new_version = self.env['project.scrum_release'].create({'name': "new",
                                                                 'color': 4,
