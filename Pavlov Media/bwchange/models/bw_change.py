...
class BandwidthChange(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'),
                             ...
                             rack_visibility='always', store=True)
    ...
    circuit_paid_by = fields.Many2one('bwchange.owner',
                                      string="Circuit Paid By")
    sold_transport = fields.Float(
        string="Sold Transport/DIA Down (OffNet Mbps)",)
    dia_upload = fields.Float(string="DIA Up (OffNet Mbps)")
    tesseractive_service = fields.Boolean(string="Tesseractive Service?")
    tesseractiv_speed = fields.Float(
        string="Tesseractiv Speed (OnNet Mbps) *If Applicable")
    ...
    distro_switch = fields.Many2one(
        'bwchange.speed',
        string="Distro Switch (Between Buildings)")
    core_switch = fields.Many2one(
        'bwchange.speed',
        string="Internal Core Switch (Site Backbone)")
    user_switch = fields.Many2one(
        'bwchange.speed',
        string="Site User Switch Port Speed")
    ...
    def create(self, vals):
        seq = self.env['ir.sequence'].next_by_code('bwchange.change') or '1000'
