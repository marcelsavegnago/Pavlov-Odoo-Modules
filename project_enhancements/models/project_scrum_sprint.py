...
#General
     ...
     goal = fields.Char(string="Sprint Goal",
                        help="Add a general 'Goal' to the sprint to help the teams understand the focus of this Sprint.")
     start_date = fields.Date(string="Start Date",
                              required=True,
                              help="When the Sprint officially starts.")
     end_date = fields.Date(string="End Date",
                            required=True,
                            help="When the Sprint officially ends.")
     progress = fields.Float(string="Percent Complete",
     ...
     total_points = fields.Integer(string="Total Points",
                                   compute="_compute_task_points",
                                   help="Computed total of all Points from Tasks.")
     project_tasks = fields.One2many('project.task',
                                     'sprint_id',
                                     string="Project Tasks",
                                     help="List of Tasks associated to this Sprint.")
     scrum_teams = fields.Many2many('project.scrum_team',
                                    string="Scrum Teams",
                                    help="List the scrum Teams that are participating in this Sprint.")
     projects = fields.Many2many('project.project',
     ...
     forecasts = fields.One2many('project.forecast',
                                 'sprint_id',
                                 string="Forecasts",
                                 help="Lists all the Forecasts from related Tasks.")
             ...
             if record.date_start and record.date_end and record.planned_hours and record.user_id and record.allow_forecast:
                 if record.forecasts:
                     for forecast_record in record.forecasts:
                         ...
                         if employee_id.id:
                              forecast_record.write({'start_date': self.start_date,
                                                     'end_date': self.end_date,
                                                     'resource_hours': record.planned_hours,
                                                     'employee_id': employee_id.id})
                 else:
                     employee_id = self.env['hr.employee'].search([('user_id', '=', record.user_id.id)])
                     if employee_id.id:
                         self.env['project.forecast'].create({'sprint_id': record.sprint_id.id,
                                                              'employee_id': employee_id.id,
     ...
     def close_and_create_sprint(self):
         ...
         new_sprint = self.env['project.scrum_sprint'].create({'name': "Sprint - " + str(sprintid),
                                                               'start_date': sprint_start,
                                                               'end_date': sprint_end,
                                                               'projects': [(6,0,project_ids)],
                                                               'scrum_teams': [(6,0,team_ids)]})
         for record in self.project_tasks:
